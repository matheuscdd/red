...
public class CreateUserHandler : IRequestHandler<CreateUserCommand, UserDto>
{
    ...
    {
        var usernameExists = await _userRepository.CheckUserNameExistsAsync(request.UserName, cancellationToken);
        if (usernameExists)
        {
        ...
        var (entity, errors) = await _userRepository.CreateAsync(
            new User(request.UserName, request.Password, request.Email),
            request.Password,
            cancellationToken
        );
        if (entity == null) 
